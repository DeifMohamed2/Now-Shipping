/**
 * Admin edit-order.js — 4-step wizard for admin order edits.
 */
(function () {
  'use strict';
  ...
  function updateFeeSummary() {
    ...
    var totalFee = state.lastFee || S.orderFees || 0;
    var baseFee = isExpress ? Math.min(100, totalFee) : totalFee;
    var expressAdd = isExpress ? Math.max(0, totalFee - baseFee) : 0;
  ...
  function bindFormSubmit() {
    var form = el('adminEditOrderForm');
    if (!form) return;
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch('/admin/orders/edit-order/' + orderId, {
        method:  'PUT',
          ...
          Swal.fire({
            ...
            confirmButtonText: t('swalViewOrder', 'View order'),
              ...
              if (result.isConfirmed && res.data && res.data.order && res.data.order.orderNumber) {
                window.location.href = '/admin/order-details/' + res.data.order.orderNumber;
            } else {
